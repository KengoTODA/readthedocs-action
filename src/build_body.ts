...
export default function buildBody(
  ...
  if (languages.length === 1) {
    const url = `https://${escape(project)}.readthedocs.io/${
      languages[0]
    }/${escape(branch)}/`;
    // https://docs.readthedocs.io/en/latest/badges.html
    const badge = `https://readthedocs.org/projects/${escape(
      project
    ...
    languages.forEach((language) => {
      body += `${language}: https://${escape(
        project
      )}.readthedocs.io/${language}/${escape(branch)}/\n`;
    });
  }
  return body + "\n" + END;
}
